...
| `import_jobs` | Table | Added 2026-09-11. **A lease queue, the twin of `marking_jobs`, one stage earlier** — see below. Row `_id` is an opaque generated string; unlike `marking_jobs` the operation id (`{definitionId}:{version}:{sourceHash}:{parsePromptVersion}`) is a plain `operationId` column with its own unique index `ux_import_jobs_operation` → Postgres `UNIQUE` constraint, so a retried upload is a rejected duplicate insert rather than a second job. Claim index `ix_import_jobs_due` on `(state, nextAttemptAt)`. `stage` (`Extracting` → `Done`) records how far a paid parse got and is never reset by `RetryAsync` or `FailAsync` — only `AdvanceAsync`, called by a live lease holder, moves it forward |

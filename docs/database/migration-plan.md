...
| `import_drafts` | Table + `jsonb` for findings/warnings | `S6b`. No domain aggregate is stored — only `packageJson`; `ExamVersion` is reconstructed on read by re-validating it, so this table carries no persistence attributes on the domain type. Row id is `ExamImportWorkflow.StableDraftId`, a hash of `(definitionId, versionNumber, route, packageHash)` → Postgres `PRIMARY KEY (id)` with an `ON CONFLICT DO NOTHING` insert, the same collision-is-a-replay idempotency shape as `usage_ledger`. `revision` is the compare-and-swap counter for `ReplaceAsync`, the same pattern as `answer_sheets`' `Revision` — a conditional `UPDATE … WHERE revision = $expected`, never a plain `UPDATE`. Index `ix_import_drafts_definition_version` |

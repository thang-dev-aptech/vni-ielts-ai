...
| `exam_packages` | Table + `jsonb` for findings/entries | Package upload lifecycle (`Uploaded` → validate → parse → review → confirmed/rejected). `version` is the optimistic-concurrency counter for status transitions; `uploadRef`/`sha256` point at object storage — the bytes are not in Mongo. `createdVersionIds` becomes a join or `text[]`. Retention uses durable `Pending`/`Deleting`/`Purged` state; it sets `uploadPurged` only after idempotent object deletion while the metadata row remains. Stale `Deleting` claims are retryable |

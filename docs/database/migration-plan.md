...
Thirty-six collections, read from `MongoContext` and `AuditLog` — twelve on 2026-08-28, plus `content_sources` added on 2026-08-29 with the content rights registry (`FS0.1`), plus `canonical_explanations` and `personalized_explanations` with the R/L explanation pipeline (`FS5`), plus `learner_goals`, `learner_activity_days` and `coaching_advice` added on 2026-09-04 with goal coaching and the study streak, plus `library_documents`, `articles` and `usage_ledger` added on 2026-09-07 with the MVP code-first slices (`S3`, `S5`), plus `import_drafts` and `import_batch_checkpoints` added on 2026-09-07 with the exam-import front door (`S6b`), plus `staff_invitations`, `privacy_requests`, `personal_data_exports` and `admin_privilege_coordination` added on 2026-09-08 with CMS staff administration (integration onto `main`), plus `exam_packages`, `parsed_exam_candidates`, `source_document_grouping_proposals`, `exam_extraction_runs` and `media_assets` added on 2026-09-08 with package upload / AI extraction / media library (`I9`/`I10`), plus `import_asset_cleanup_intents` and `import_asset_cleanup_coordination` added on 2026-09-09 with ZIP exam-asset staging, approval promotion and cleanup/approval coordination, plus `package_upload_idempotency` and `package_upload_reconciliation` added on 2026-09-10 with fenced multipart upload idempotency and failed-object-cleanup recovery (`H2`). The right-hand column is the target shape, and the notes column is where the work actually is.
...
| `library_documents` | Table | `P-22`. Independent of `exam_versions`; `relatedExamIds` is a reserved column, written only while `null`. Status is the same four-state lifecycle as `articles` |
| `articles` | Table | `P-22`. Unique `ux_articles_slug` — addressed by slug, never by id, so the unique constraint is the thing a duplicate publish actually collides on |
| `usage_ledger` | Table, append-only, no `UPDATE`/`DELETE` grant | `P-14`. Row id is the domain's own idempotency key (`grant:{userId}`, `daily:{userId}:{day}`, `referral:{inviteeId}`, or a random id for a `use` row) → Postgres `PRIMARY KEY (id)` with an `ON CONFLICT DO NOTHING` insert reproducing the same collision-is-a-no-op semantics. Balance is `SUM(turns) WHERE user_id = $1`, never a stored counter. Index `ix_usage_ledger_user_at` on `(user_id, at DESC)` |
| `import_drafts` | Table + `jsonb` for findings/warnings | `S6b`. No domain aggregate is stored — only `packageJson`; `ExamVersion` is reconstructed on read by re-validating it, so this table carries no persistence attributes on the domain type. Row id is `ExamImportWorkflow.StableDraftId`, a hash of `(definitionId, versionNumber, route, packageHash)` of the JSON, not media bytes → Postgres `PRIMARY KEY (id)` with an `ON CONFLICT DO NOTHING` insert, the same collision-is-a-replay idempotency shape as `usage_ledger`. `revision` is the compare-and-swap counter for `ReplaceAsync`, the same pattern as `answer_sheets`' `Revision` — a conditional `UPDATE … WHERE revision = $expected`, never a plain `UPDATE`. `assetManifest` is `jsonb` of canonical `assets/` references plus staging key, content type, length and SHA-256; absent BSON maps to an empty list. Index `ix_import_drafts_definition_version` |
| `import_batch_checkpoints` | Table | `S6b`. Resume state for `ImportBatchRunner`. Row id is `{batchId}:{itemId}` → Postgres `PRIMARY KEY (id)`; writes are a plain upsert (`INSERT … ON CONFLICT (id) DO UPDATE`), not a CAS — the runner is the only writer of one pair at a time. Index `ix_import_batch_checkpoints_batch` |
| `import_asset_cleanup_intents` | Table (durable intent) | Written when ZIP exam media was staged or promoted in object storage and the matching Mongo draft/catalogue write then failed (or was abandoned). Fields: `draftId`, `references[]` (canonical `assets/` paths only), bounded `reason` enum (`DraftSaveFailed` / `ApprovalCommitFailed` / `StagingAbandoned` — never `Exception.Message`), `status=pending`. Cleanup processing must skip delete when any catalogue version still references the object. S3 and Mongo are not one transaction; the intent is the recovery pointer, same shape as `media_orphan_intents` |
| `import_asset_cleanup_coordination` | Table (single-row lease) | Coordination lock between import approval commit and cleanup deletion. Single `_id=import-assets` row with `owner` and `leaseUntil`; acquired by conditional `findOneAndUpdate` before either (a) catalogue-check+delete cleanup or (b) catalogue commit with final-asset re-verify. Prevents approving a version that points at an object deleted by concurrent cleanup |
| `staff_invitations` | Table | CMS staff invite lifecycle. Unique `ux_staff_invitations_token` on the hashed token; **partial unique** `ux_staff_invitations_live_email` so only live/pending invitations collide on email (history rows may repeat an address). Index `ix_staff_invitations_status_created`. Accept path shares a Mongo session with user/identity writes |
| `privacy_requests` | Table, FK to subject user | PDPL request queue (`PendingReview` → `Approved` → execute). Index `ix_privacy_requests_subject`. Requester must not self-approve; execute stays fail-closed (`POLICY_NOT_CONFIGURED`) until retention/erasure policy is configured (`M-55`) |
| `personal_data_exports` | Table, FK to subject user | Export request records only. Archive creation is fail-closed until retention policy exists — no ZIP is written on `POLICY_NOT_CONFIGURED` |
| `admin_privilege_coordination` | Table (coordination lease) | Serialises last-active-admin / bulk-suspend mutations: every repository write that can remove the last admin runs inside one Mongo session keyed by this document. Postgres needs an equivalent advisory lock or single-row `SELECT … FOR UPDATE` so concurrent suspend/revoke cannot both succeed |
| `exam_packages` | Table + `jsonb` for findings/entries | Package upload lifecycle (`Uploaded` → validate → parse → review → confirmed/rejected/failed). `version` is the optimistic-concurrency counter for status transitions; `uploadRef`/`sha256` point at object storage — the bytes are not in Mongo. `createdVersionIds` becomes a join or `text[]`. `importDraftId` links to `import_drafts` for checklist review and atomic catalogue promotion; `failureCode` and `failureDetail` record bounded sanitized failure information. Retention uses durable `Pending`/`Deleting`/`Purged` state; it sets `uploadPurged` only after idempotent object deletion while the metadata row remains. Stale `Deleting` claims are retryable |
| `package_upload_idempotency` | Table | Endpoint-level multipart upload idempotency keyed by actor and key. Unique composite index `ux_package_upload_idempotency_actor_key` on `(actorId, idempotencyKey)` → composite `UNIQUE` constraint. Stores `sha256`, `packageId`, `status`, `statusCode`, and `createdAt`. Same actor + key + same bytes replays stored response; same actor + key + differing content hash raises conflict |
| `package_upload_reconciliation` | Table (durable cleanup pointer) | Written only when multipart intake saved the deterministic GridFS object but package/idempotency completion failed and compensating object deletion also failed. Stores package/upload references plus a bounded cleanup reason; never stores signed URLs, credentials, raw exception messages, or uploaded bytes. Reconciliation can retry idempotent deletion independently of the failed request token |
| `parsed_exam_candidates` | Table + `jsonb` for modules/sources | AI- or parser-produced candidate awaiting human review. Index `ix_parsed_exam_candidates_package` on `packageId`. `confidence` is `Decimal128` → `numeric`. Confirm path may create an `exam_versions` draft in the same transaction as the status flip |
| `source_document_grouping_proposals` | Table, PK = `packageId` | One proposal document per package (`_id` is the package id). `groups` is a small nested list of directory paths and entry paths → child table or `jsonb`. `version` is CAS for replace-on-review. Mongo's `groupingRetentionClaimAt` is transactional scratch used only to force write-conflict detection during purge; it is not durable domain state and needs no PostgreSQL column |
| `exam_extraction_runs` | Table (append-oriented audit) | Provider call metadata only — package/candidate ids, provider, model, prompt/schema versions, token counts, hashes, timestamps. **No source text, prompt body, answer key, or credential columns.** FK-ish to `exam_packages` / optional candidate id |
| `media_assets` | Table | CMS media library metadata; object bytes live in object storage under a server-generated `storageKey`. Index `ix_media_assets_uploaded_at`. `retired` soft-delete; never trust a client-supplied filename as the storage key |
| `media_orphan_intents` | Table (durable intent) | Written when media upload Put succeeded, metadata Save failed, **and** compensation Delete also failed. Fields: `storageKey`, `mediaId`, `fileName`, bounded `errorCode` (`MEDIA_CLEANUP_*` — never a raw exception message), `status=pending`. The intent commits independently as the primary recovery pointer; `audit_log` (`MediaOrphanCleanupFailed`) is best-effort and must never roll the intent back. Reconcile discovers pending intents, deletes the object, and closes the intent |

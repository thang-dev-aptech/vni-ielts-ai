...
| `dictation_attempts` | Table, append-only, no `UPDATE`/`DELETE` grant | One row per attempt at one sentence — `M-22`. Modelled on `usage_ledger` and for the same reason: a learner replays a sentence until they get it, so progress is a `SELECT` over rows rather than a stored counter a failed write could desynchronise. Row id is random, not derived: two identical tries are two events, so there is deliberately **no** idempotency collision here, unlike `usage_ledger`. What the learner typed is **not** stored — a personal-data class with no retention decision behind it (`G-11`); adding it later is a nullable column and breaks nothing already written. `is_perfect` is stored rather than derived from `correct = total`, so a later change to the comparison rule cannot rewrite a verdict a learner already saw. Index `ix_dictation_attempts_user_set` on `(user_id, set_id, is_perfect)` serves both reads — the second by prefix |
...
| `import_jobs` | Table | Added 2026-09-11. **A lease queue, the twin of `marking_jobs`, one stage earlier** — see below. Row `_id` is an opaque generated string; unlike `marking_jobs` the operation id (`{definitionId}:{version}:{sourceHash}:{parsePromptVersion}`) is a plain `operationId` column with its own unique index `ux_import_jobs_operation` → Postgres `UNIQUE` constraint, so a retried upload is a rejected duplicate insert rather than a second job. Claim index `ix_import_jobs_due` on `(state, nextAttemptAt)`. `stage` is stored as `ImportJobStage`'s underlying **int**, not its name (`0 Extracting` → `6 Done`) — the comparison that guards it, below, is an integer `<=`, not a string sort — and is never reset by `RetryAsync` or `FailAsync`. `AdvanceAsync` is the only writer, and it is itself guarded: the update's filter requires the stored `stage` to be at or before the requested one, in the same statement as the lease check, so a stale or mistaken rewind is refused (`false`, nothing written) rather than merely discouraged by caller discipline |
...
**`import_jobs` is the same lease queue, one stage earlier — with one guard the twin does not need.** Same shape as `marking_jobs` — `ClaimAsync` is one `FindOneAndUpdateAsync` over `state in (Pending, Retryable, Running)` and `nextAttemptAt <= now` and (`leaseUntil` null or in the past) — and every other transition (`RenewAsync`, `AdvanceAsync`, `CompleteAsync`, `RetryAsync`, `FailAsync`) additionally filters on the caller's own `leaseToken`, returning whether the filter matched. A worker that keeps writing after losing that boolean check is a worker paying for a second Cambridge parse. `RetryAsync` and `FailAsync` clear the lease but never touch `stage`. `AdvanceAsync` goes one step further than its twin: `marking_jobs` has no stage field to guard, because marking pays for one provider call and a rewind there would cost nothing — an import pays separately for the parse, the transcription and forty explanations, so `AdvanceAsync`'s filtered update additionally requires `stage <= requested stage`, refusing a rewind in the same statement rather than trusting the caller not to attempt one.

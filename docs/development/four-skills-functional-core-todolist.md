...
- **Đang thực hiện:** — (queue đóng)
- **Phase hiện tại:** Functional Core Ready — Speaking AI deferred
- **Functional Core Ready:** đạt 29/08/2026
...
- [x] **FS1 — Exam Package v2 và ResponseSlot**
- [x] **FS2 — Import/conversion pipeline và pilot content**
- [x] **FS3 — PracticeUnit catalogue và session scope**
- [x] **FS4 — Reading/Listening practice runner** — đóng 29/08/2026 (phase gate browser E2E xanh)
- [x] **FS5 — Deterministic results và AI explanation** — đóng 29/08/2026 (phase gate xanh)
- [x] **FS6 — Writing runner và GPT/Gemini marking** — đóng 29/08/2026 (recorded gate xanh; live smoke conditional)
- [x] **FS7 — Mock state machine bốn kỹ năng** — đóng 29/08/2026 (phase gate xanh; browser mock E2E)
- [x] **FS8 — Speaking capture và Cloudflare R2** — đóng 29/08/2026 (phase gate xanh; R2 live smoke conditional)
- [x] **FS9 — Hardening, E2E và Functional Core certification** — đóng 29/08/2026 (FS9.6 final report)
...
- [x] **FS1.1 · Schema v2 additive/migratable**
...
- [x] **FS1.3 · v1 compatibility/migration**
...
- [x] **FS2.1 · Hai đường nhập tách biệt**
...
- [x] **FS2.2 · Safe source extraction**
...
- [x] **FS2.3 · Provider-neutral parser**
...
- [x] **FS2.6 · Cambridge batch readiness**
...
- [x] **FS3.1 · PracticeUnit projection**
...
- [x] **FS3.3 · Start-session contract**
...
- [x] **FS4.3 · Footer theo ResponseSlot**
...
- [x] **FS4.7 · Autosave/offline**
  - Patch theo `responseSlotId`, revision + per-slot sequence, offline journal và final-save barrier trước
  navigation/submit.

### Phase gate FS4

- [x] Component/a11y tests cho mọi renderer và keyboard-only drag/drop — evidence: `QuestionInput.test.tsx`, `exam-flow.test.tsx`, `practice-runner.test.tsx` (FS4.6 addendum).
- [x] Browser E2E desktop + mobile hoàn thành một Reading part và một Listening part — `practice-runner.spec.ts` ×2 projects; **22/22** FS4-focused specs xanh (`pnpm exec playwright test tests/practice-runner.spec.ts tests/offline.spec.ts tests/races.spec.ts` → exit **0**, 29/08/2026). Harness: `findSyntheticPartUnit` filters `SYNTHETIC_EXAM` title; `showPracticeQuestions` opens mobile Questions tab.
- [x] Fault tests: save out-of-order, offline/reconnect, reload giữa part, submit double-click — `races.spec.ts` / `offline.spec.ts` / `practice-runner.spec.ts` practice variants xanh trên desktop + mobile (cùng run exit 0).
- [x] Audio 404/range failure trên practice route — **N/A** trên synthetic fixture (no `audioKey`); FS4.5 unit/integration player fault tests remain the evidence layer.
- [x] Pre-submit network: answer key/transcript/explanation absent — browser `assertNoPreSubmitLeaks` xanh trên desktop + mobile; integration backstop `Pre_submit_session_exposes_slots_but_not_keys_or_explanations`.

**Gate closed 29/08/2026:** `dotnet build backend/src/Vni.Ielts.Api/Vni.Ielts.Api.csproj` → exit 0; `cd e2e && pnpm typecheck` → exit 0; FS4-focused Playwright → exit 0 (22/22). Full `pnpm e2e` → 30/34 (4 late non-FS4 failures = auth `RATE_LIMITED` after many registrations — not FS4 gate blockers). Ports 5199/5273, DB `vni_ielts_e2e`, Mongo `:27018`, `PLAYWRIGHT_BROWSERS_PATH` → `%LOCALAPPDATA%\ms-playwright`.
...
- [x] **FS5.6 · Failure semantics**
  - Deterministic result hiện ngay; explanation pending/failed độc lập và retry được.

### Phase gate FS5

- [x] Perfect/wrong/blank/variant/word-limit/multi-slot matrices cho R/L.
  ```
  2026-08-29 — `dotnet test …Domain.Tests --filter FullyQualifiedName~DeterministicScorer|…AnswerMatcher`
  → exit 0 (34/34). Covers perfect/blank/variant/word-limit/multi-slot + matcher matrices.
  ```
- [x] Một model response cố thay correct answer/band bị schema/application từ chối.
  ```
  `ExplanationOutputValidatorTests` (Band_field_is_refused, Wrong_correct_answer_is_refused,
  Invalid_band_value_like_6_3…) trong filter Application explanations → exit 0 (11/11 with cache/failure/evidence).
  ```
- [x] Cache chứng minh cùng canonical question không gọi provider lại cho mỗi learner.
  ```
  `CanonicalExplanationWorkflowTests.Cache_prevents_second_provider_call_for_same_question` — same 11/11 run.
  ```
- [x] Provider timeout không trì hoãn hoặc làm mất deterministic score.
  ```
  `ExplanationFailureSemanticsTests.Deterministic_score_returned_while_explanation_job_failed` — same 11/11 run.
  ```
...
- [x] **FS6.7 · Initial evaluation set**
...
- [x] **FS6.8 · Production controls**
  - Feature flag, primary/fallback có kiểm soát, budget/rate limit, outbox retry/dead-letter và learner
  status `pending|completed|failed`.

### Phase gate FS6

- [x] Recorded-response tests của cả GPT/Gemini; prompt injection trong essay không phá output contract.
  ```
  2026-08-29 — `dotnet test …Infrastructure.Tests --filter FullyQualifiedName~Ai.Writing`
  → exit 0 (20/20). Includes `WritingGoldenSeedTests` OpenAi+Gemini + `Prompt_injection_in_essay_is_stripped…`.
  ```
- [x] Evidence giả, thiếu criterion, band 6.3, model-reported average sai đều bị refuse.
  ```
  `WritingEvaluationValidatorTests` (band 6.3, fake evidence) + `CriterionMarkingTests` (missing criterion,
  recomputed band wins) → Domain exit 0 (21/21); covered in Ai.Writing 20/20.
  ```
- [x] Worker restart/duplicate delivery chỉ lưu một marking; result pending chuyển completed mà không reload
  mất dữ liệu.
  ```
  `MarkingOutboxTests` (VNI_REQUIRE_MONGO=1) → exit 0 (8/8); `SectionMarkingRunnerTests`
  `A_task_that_is_already_marked_is_not_evaluated_a_second_time` → exit 0 (13/13).
  ```
- [x] Live synthetic smoke là conditional gate khi key được nạp; không dùng learner data qua reseller.
  ```
  **Conditional pending** — no OpenAI/Gemini keys in environment. `AiEgressTests` → exit 0 (18/18)
  prove reseller+learner refusal; live smoke not claimed.
  ```
...
- [x] **FS7.1 · Package-driven sequence**
...
- [x] **FS7.5 · Resume/recovery**
  - Reload/device reconnect mở đúng skill/part; closed session vào results; retry không advance hai lần.

### Phase gate FS7

- [x] E2E chạy một mock rút gọn qua bốn skill với fake evaluator/transcript adapter.
  ```
  2026-08-29 — `e2e/tests/four-skills-mock.spec.ts` (3 specs × desktop+mobile = 6).
  ...
  Follows server `moduleSequence`; R/L filled; Writing essays autosaved; Speaking mic skipped;
  overallBand null + UI overall-pending; no Writing/Speaking bands.
  ```
- [x] Production-default no-voice run kết thúc trung thực với Speaking/overall pending, không crash và không
  band giả.
  ```
  2026-08-29 — `ExamRunContractTests.Full_mock_through_four_skills_ends_with_speaking_and_overall_pending`
  (+ sequence/reload Full_mock slice) with `VNI_REQUIRE_MONGO=1` → exit 0 (8/8 filter).
  `SequenceProfileTests` → exit 0 (4/4).
  Browser: four-skills-mock full-path asserts overallBand null + Chưa chấm / Not marked (6/6).
  ```
- [x] Race tests submit/advance/expiry/double-click và worker duplicate đều giữ đúng một transition/marking.
  ```
  Integration: replayed advance, deadline read, late submit (in 8/8 slice). Application:
  `ExamLifecycleTests` expired-twice / race / second-advance → exit 0 (4/4). Outbox worker duplicate:
  `MarkingOutboxTests` → exit 0 (8/8). Browser: Next dblclick (held advance) + Submit dblclick →
  one transition each (desktop+mobile in four-skills-mock.spec.ts). Footer remount on module change
  in ExamRunnerPage prevents post-transition skip.
  ```
...
- [x] **FS8.2 · R2-compatible private store**
...
- [x] **FS8.4 · Web recorder**
  ...
  - Evidence (2026-08-29): `SpeakingRecorder` + `recordingDraft` IndexedDB; meter/progress/queued/
  re-record UX; multipart 503 kept; vitest 18/18 on recorder+draft+contract (see report addendum).
- [x] **FS8.5 · Native seam**
  ...
  - Evidence: `@vni/speaking-audio` port (`SpeakingAudioCapture`) + `WebSpeakingAudioCapture` +
    `DeferredNativeSpeakingAudioCapture` stub; `SpeakingRecorder` consumes the port; native Android/iOS
    plugin deferred (see `plugins/speaking-audio/README.md`).
- [x] **FS8.6 · Retention/deletion/reconciliation**
...
- [x] **FS8.7 · No-voice result state**
  - Recording hoàn tất nhưng chưa có ASR trả `AwaitingVoiceProvider`; có hướng dẫn rõ cho learner/admin.

### Phase gate FS8

- [x] MinIO integration: init→upload→complete→HEAD→sheet link; checksum/size/type sai bị từ chối.
  ```
  2026-08-29 — `dotnet test …Infrastructure.Tests --filter FullyQualifiedName~SpeakingRecordingUploadTests`
  -p:RunAnalyzers=false → exit 0 (8/8). Happy path + Complete_rejects_checksum_mismatch /
  Complete_rejects_size_mismatch_against_init; Init_rejects_non_audio / oversized in AbuseTests.
  ```
- [x] Negative tests: traversal key, upload của user khác, expired URL, frozen section, orphan/re-record và
  R2/MinIO unavailable.
  ```
  Same MinIO 8/8 (traversal, other-learner, Expired_presigned_put_url_is_refused_by_minio, stale abort,
  re-record) + AbuseTests (store unavailable, size/type/checksum/owner/traversal) +
  ExamLifecycleTests.A_recording_meeting_a_frozen_sheet_is_refused… + Remainder orphan/re-record.
  Combined Application filter → exit 0 (34/34).
  ```
- [x] Real-server upload file >1 MiB; TestServer-only test không được dùng làm bằng chứng cho
  Kestrel body limit.
  ```
  `dotnet test …Integration.Tests --filter FullyQualifiedName~KestrelTransportTests`
  (VNI_REQUIRE_MONGO=1) → exit 0 (3/3). Real Kestrel loopback — not TestServer. Proves 3 MiB multipart
  accepted and over-ceiling refused. Playwright browser CORS direct-PUT to MinIO/R2 not claimed.
  ```
- [x] R2 live smoke là conditional gate khi owner nạp key; report chỉ ghi pass/fail, không ghi endpoint/key.
  ```
  **Conditional pending** — no owner R2 credentials. MinIO is the contract-test target (8/8 above).
  ```
...
- [x] **FS9.1 · Security/privacy**
  - Authorization/IDOR, answer-key leak, upload abuse, model prompt injection, secret scan, retention và
  audit redaction. Đóng 29/08/2026 — negative tests + hardenings; residual GitGuardian R18 /
  security-fixture CI remain documented non-pass.
- [x] **FS9.2 · Accessibility/responsive** — đóng 29/08/2026 (jsdom); browser
  mobile/zoom vẫn phụ thuộc FS4 Playwright gate.
  - Keyboard/screen reader, reduced motion, colour-independent states, phone/tablet/desktop và long
  content/zoom.
  - **jsdom proven:** footer `aria-disabled` section steps, target `aria-controls` hygiene, footer box
  names by state, connection text channel, confirm unanswered `role="status"`, essay under-min text.
  - **Browser mobile/zoom evidence:** deferred to FS4 phase-gate Playwright `desktop`/`mobile`
  projects (same dependency already recorded on FS4 gate — not re-proven here).
- [x] **FS9.3 · Performance/reliability**
...
- [x] **FS9.4 · Full regression** — đóng 29/08/2026 (certification suite; browser E2E closed via FS4/FS7 gates same day)
  ...
  - Evidence (2026-08-29) — command + exit code:

    | Command | Exit | Result |
    |---|---:|---|
    | `node scripts/check-docs.mjs` | 0 | All documentation checks passed |
    | `node scripts/check-generated-drift.mjs --mode=all` | 0 | After OpenAPI + api-client regen |
    | `dotnet build backend/Vni.Ielts.sln -p:RunAnalyzers=false` | 0 | 0 warnings / 0 errors |
    | `dotnet test …Domain.Tests` | 0 | 198 passed, 0 skipped |
    | `dotnet test …Application.Tests` | 0 | 249 passed, 0 skipped |
    | `dotnet test …Infrastructure.Tests` | 0 | 164 passed, 0 skipped (MinIO reachable) |
    | `VNI_REQUIRE_MINIO=1` filter SpeakingRecording+S3ObjectStore | 0 | 12/12 required, 0 skipped |
    | `VNI_REQUIRE_MONGO=1 VNI_REQUIRE_MINIO=1 dotnet test …Integration.Tests` | 0 | 208 passed, 0 skipped |
    | `pnpm --filter @vni/web typecheck` | 0 | clean |
    | `pnpm --filter @vni/web exec vitest run` | 0 | 298 passed (31 files) |
    | Browser Playwright FS4-focused | 0 | **22/22** (`practice-runner` + `offline` + `races`) |
    | Browser Playwright FS7 mock | 0 | **6/6** (`four-skills-mock` × desktop/mobile) |

  - Fixes applied this pass: regenerate `contracts/openapi/v1.json` + api-client; `ExamRunContractTests` SSO token reuse (auth 120/min flake).
  - Residual non-pass (known): GitGuardian R18; security-fixture CI; full `pnpm e2e` 30/34 late auth `RATE_LIMITED` on non-FS4 specs.
- [x] **FS9.5 · Operational docs**
...
- [x] **FS9.6 · Final report** — đóng 29/08/2026
  - Capability matrix, test counts, live-smoke status, known limitations và deferred voice backlog
  recorded in `four-skills-functional-core-report.md` § FS9.6 + `_workspace/workflow/final-report.md`.

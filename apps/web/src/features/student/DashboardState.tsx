...
export function StatStrip({ sittings }: { sittings: SittingSummary[] }) {
   ...
   *
   * <b>`overallBand` is a real number again since `W1`</b> — the history query
   * loads the markings and the jobs, so a finished mock carries the same band
   * the results screen shows. This tile still does not read it, and that is
   * deliberate: an overall band exists only for a Full sitting, so a learner
   * whose recent history is single-skill practice would see this tile empty
   * where the per-skill band below it is not. Which number belongs in the
   * headline slot is a product question, and `W7` is where the history row
   * learns to say which skills its band covers. Until then the honest reading
   * of this list is the per-skill band it already shows.
   * → `H-8`, `A-11`, `Q-01`
   */

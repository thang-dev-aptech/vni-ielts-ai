...
import { roleLabel, permissionLabel } from '../lib/permissions.js';
  ...
  return (
    <>
      <header className="cms-head">
        <h1>Vai và quyền</h1>
        <p>
          {roles.length} vai trò CMS, {permissions.length} quyền. Ma trận chỉ đọc; gán vai trò tại trang người dùng.
        </p>
      </header>
                  ...
                  <tr key={key}>
                    <td>
                      ...
                      {permissionLabel(key) !== key && (
                        <span className="cms-sub"> {permissionLabel(key)}</span>
                      )}
                    </td>
                    {roles.map((role) => (

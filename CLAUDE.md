...
| Content VNI may publish — no VNI-owned exam exists yet | **`Q-03` (18/09/2026) narrows `P-21`: the papers already in use are published and the exposure accepted.** `ContentRightsPolicy` is unchanged and still refuses without a `RightsProof`; `ContentRights:AllowPublicationWithoutProvenRights` records the acceptance, startup announces it, and every such publication is audited as an override. **An accepted risk under `M-53`, not a licence — never describe it as resolved.** There is still no way to record a genuine grant: nothing writes a `RightsProof`, so a real licence would have to be inserted by hand |
...
> **A canonical document that is wrong about the code is worse than no document:** it sends the next reader, or the next agent, to build something that already exists or to trust something that does not. This inventory was rewritten after slices `S1`…`S8` landed, and again on 2026-09-09 for Writing results + marking.

**Built and running:** identity (registration by name + phone + password, one sign-in field accepting a phone number *or* an email address, Google SSO, device management — **email verification and all mail infrastructure were removed on 2026-09-08**, and the address is now a mutable label that moves an account rather than a permanent identity → [ADR-0018](docs/decisions/0018-email-as-a-movable-account-label.md)); the exam engine (catalogue, sittings, autosave with per-question ordering and an offline journal, Full Test advance, expiry, server-authoritative timer); deterministic Reading and Listening marking; the Writing AI marking path, live with real learner essays since 2026-09-03 through the `api.vietapi.tech` / `apithat.dev` reseller (owner decision 02/09/2026, *"cho chạy thật luôn"* — a data-processing agreement still does not exist and the reseller's real backend is unverified, see `B-2`, `M-28`, [`docs/development/ai-provider-setup.md`](docs/development/ai-provider-setup.md)); **Writing results UI** (`WritingResults`, poll 8s × 40, `P-13` label on every band, combined band from server `writingBand` never a client mean); **three-layer marking** (admission in Domain → `ISectionEvaluator` → limiter caps + provenance) described in [`docs/ai/writing-marking.md`](docs/ai/writing-marking.md) — develop secrets still pin `ielts-writing-synthetic-v1`; HttpClient Writing timeout **floors at 180s**; Failed jobs are not reopened by «Kiểm tra lại»; combined Writing band via configured `Assessment:Writing:TaskWeights` 1:2 (`WritingTaskWeightPolicy`, `P-12`); Speaking recording upload **and** playback (`GET …/sessions/{id}/recordings/{questionId}/playback`); post-submit exam content on session results (`SessionResultsView.Content`, gated off `InProgress`); **the Listening transcript rendered** on the results screen (`PartView.transcript` had been arriving since 2026-09-11 with nothing reading it — a Listening part carries no `body`, so the panel opened on a title and nothing else); **dictation attempts kept** (`dictation_attempts`, append-only like the usage ledger; the library card reads "4/6 câu đúng" and the practice header carries the previous best — what the learner typed is deliberately not stored, `G-11`); the append-only usage ledger (`UsageEntry` / `UsageRecorder` / `GET /api/v1/me/usage`, 10-turn grant keyed on the provider subject for socially-created accounts, daily-login and referral earn seams — referral now confirms at registration, and amounts beyond the grant stay at 0 under `G-11`); Documents and Articles as real collections with learner + admin APIs and CMS screens (hard-coded catalogue arrays removed); ZIP safety inspector, role-aware since 2026-09-10 — `reading/` and `listening/` each split into a paper folder and a key folder, and the key is read by code and never sent to the AI parser, closing the hole measured on 2026-09-02 — plus a four-layer deterministic cross-check between the paper and its key (counting, shape legality, the paper's own stated rules, passage anchoring; [`docs/development/answer-key-cross-check.md`](docs/development/answer-key-cross-check.md)) + `POST /api/v1/admin/import/packages` (multipart `file`) with draft persistence, warning override + audit, and `ImportReviewWorkflow.ApproveAsync` refusing `IMPORT_FINDINGS_BLOCKING` with no override when any finding is severity `error`; exam review lifecycle `Draft → InReview → Approved → Published` / `Unpublished`, permissions `exam.submit` / `exam.review`, reviewer ≠ author when `AuthorId` is known; admin Roles matrix columns from `GET /api/v1/admin/roles` `permissions[]` (`PermissionKeys.All`, 31 keys — the earlier "34" counted the four `SystemRoles` names in the same file); operator password reset (`POST /api/v1/admin/users/{id}/password`, `user.reset-password`, audited as `UserPasswordReset`) — the only recovery path, since registration collects no address, and confirmed as such by `Q-02`; **the password it sets is marked `MustChangePassword` and `RequireAuth` holds the learner on a change screen until they replace it**, because an operator-typed password travels through a chat message and at least two people know it; the learner web app (`apps/web`); the CMS on real APIs for users, roles, audit, exams, library, import; the whole production surface — S3-compatible object storage, startup configuration gate, liveness/readiness, Docker images, encrypted backup with a drilled restore, a generated OpenAPI contract with a drift gate, a real-browser suite. The import *engine* — `ExamImportWorkflow`, `ImportReviewWorkflow`, `ImportBatchRunner`, `FabricatedAnswerKeyGuard`, `CambridgeAnswerKeyNormalizer`, `SafeSourceDocumentExtractor`, `ExamPackageReader` — runs from the command line and is reused by the HTTP door, never rewritten.
...
**Not built, and still in the MVP-adjacent queue:** a dictation authoring surface (attempts are persisted since 2026-09-18, but the catalogue itself is still fixture-backed — `FixtureDictationCatalogue`) · batch-import HTTP surface (checkpoint store exists; no `/admin/import/batches` yet) · threading `ExamVersion.AuthorId` through the import pipeline so reviewer ≠ author applies to uploaded drafts ([open question](docs/requirements/assumptions-and-open-questions.md); [ADR-0017](docs/decisions/0017-exam-version-author-review-import-seams.md)) · the exam-version ⇄ media mapping (which published version references which media asset — `MediaLibraryPage`'s "đang dùng ở đâu" column shows a dash until it exists; the delete path must consult it before removing bytes).

**Media library — built and cut over (2026-09-11).** `media.read` / `media.upload` / `media.retire` are seeded (34 keys in `PermissionKeys.All`; editor holds read+upload, never retire). `GET|POST /api/v1/admin/media`, `GET …/{id}/content`, `POST …/{id}/retire`, `DELETE …/{id}` serve the CMS (`AdminMediaEndpoints.cs`), every action audited as `MediaUploaded` / `MediaRetired` / `MediaDeleted`. The server sniffs content type from the file's own magic bytes — the client's pre-flight is advice, never truth (WEBP is checked before RIFF; the client table is not). `MediaLibraryPage` reads the API; `previewStore` is deleted. Blobs: S3-compatible bucket `vni-media` when object storage is configured, a Development `LocalMediaBlobStore` folder when not, one port (`IMediaBlobStore`) under both.
...
5. **No AI provider type may appear in the domain layer.** Domain and Application reference a port (`ISectionEvaluator`, `ISpeechRecognizer`, …). Vendor SDKs exist only in Infrastructure adapters. → [ADR-0005](docs/decisions/0005-ai-provider-abstraction.md)
